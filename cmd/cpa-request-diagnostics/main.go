...
var runtime = pluginRuntime{logger: hostLogger{}}

const maxCallbackBytes = 64 << 20

var passThroughEnvelope = []byte(`{"ok":true,"result":{}}`)
...
func cliproxyPluginCall(method *C.char, request *C.uint8_t, requestLen C.size_t, response *C.cliproxy_buffer) (result C.int) {
	...
	defer func() {
		if recover() != nil {
			writeResponse(response, passThroughEnvelope)
			result = 0
		}
	}()
	if method == nil {
	...
	if uint64(requestLen) > maxCallbackBytes || uint64(requestLen) > math.MaxInt32 {
		writeResponse(response, passThroughEnvelope)
		return 0
	}
	var raw []byte
	...
	writeResponse(response, runtime.call(C.GoString(method), raw))
	return 0
}
...
func (hostLogger) Log(callbackID, level, message string, fields map[string]any) {
